...
int BlockHash::read(const uint64_t pos)
{
  this->blocks_used++;
  ...
  for (uint32_t i = 0; i < (BlockHash::MAX_SIZE / sizeof(Record)); i++)
  {
...
void BlockHash::reset()
{
  ...
  this->records = (const Record **)malloc(BlockHash::MAX_SIZE); // Alocate new block
